...
fn main() {
    let file_path = r"C:\Users\Administrator\Desktop\rust_url.txt";
    // let file_path = r"C:\Users\Administrator\Desktop\rust_url3+.txt";

    let urls = read_lines_from_file(file_path).unwrap();
    let l1 = urls.len();

    let unique_urls: HashSet<_> = urls.into_iter().collect();  // 使用 HashSet 对 urls 去重
    let l2 = unique_urls.len();
    // println!("{}--->{}",l1,l2); // 1795365--->722789

    let file_path = r"C:\Users\Administrator\Desktop\rust_url2.txt"; // 保存的文件路径
    ...
    for (i,k) in unique_urls.iter().enumerate(){
